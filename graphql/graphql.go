...
// Package graphql provides a GraphQL interface to Ebakus node data.
package graphql
...
// Account represents an Ebakus account at a particular block.
type Account struct {
...
// Transaction represents an Ebakus transaction.
...
func (t *Transaction) GasPrice(ctx context.Context) (hexutil.Big, error) {
	...
	return hexutil.Big(*big.NewInt(int64(tx.GasPrice()))), nil
}
...
func (b *Block) Miner(ctx context.Context, args BlockNumberArgs) (*Account, error) {
	...
	// Get the coinbase
	coinbase, err := b.backend.GetBlockAuthor(header)
	if err != nil {
		return nil, err
	}
...
type CallData struct {
	From     *common.Address // The Ebakus address the call is from.
	To       *common.Address // The Ebakus address the call is to.
...
func (r *Resolver) GasPrice(ctx context.Context) (hexutil.Big, error) {
	...
	return hexutil.Big(*big.NewInt(int64(*price))), err
}
