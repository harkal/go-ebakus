...
// PublicEbakusAPI provides an API to access Ebakus related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEbakusAPI struct {
	b Backend
}

// NewPublicEbakusAPI creates a new Ebakus protocol API.
func NewPublicEbakusAPI(b Backend) *PublicEbakusAPI {
	return &PublicEbakusAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicEbakusAPI) GasPrice(ctx context.Context) (*float64, error) {
	price, err := s.b.SuggestPrice(ctx)
	return price, err
}

// ProtocolVersion returns the current Ebakus protocol version this node supports
func (s *PublicEbakusAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicEbakusAPI) Syncing() (interface{}, error) {
...
func (s *PrivateAccountAPI) SignTransaction(ctx context.Context, args SendTxArgs, passwd string) (*SignTransactionResult, error) {
	...
	if args.WorkNonce == nil {
		return nil, fmt.Errorf("workNonce not specified")
	}
	if args.Nonce == nil {
...
// Sign calculates an Ebakus ECDSA signature for:
// keccack256("\x19Ebakus Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19Ebakus Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Ebakus signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and ebakus.
...
// PublicBlockChainAPI provides an API to access the Ebakus blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Ebakus blockchain API.
...
func (s *PublicBlockChainAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	...
	if block != nil && err == nil {
		...
		if err == nil && number == rpc.PendingBlockNumber {
			...
			for _, field := range []string{"hash", "nonce", "miner", "producer"} {
				response[field] = nil
			}
		}
		...
		for _, d := range delegates {
			delegateIds = append(delegateIds, d.Id)
		}

		response["delegates"] = delegateIds
...
func DoCall(ctx context.Context, b Backend, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides map[common.Address]account, vmCfg vm.Config, timeout time.Duration, globalGasCap *big.Int) ([]byte, uint64, bool, error) {
	...
	msg := types.NewMessage(addr, args.To, 0, value, gas, big.NewInt(0), data, false)
	...
	evm, vmError, err := b.GetEVM(ctx, msg, state, ebakusState, header)
	if err != nil {
		return nil, 0, false, err
	}
...
func DoSuggestVirtualDifficulty(b Backend, ebakusState *ebakusdb.Snapshot) (float64, error) {
	...
	for from, txs := range pending {
		for _, tx := range txs {
			dv := tx.VirtualDifficulty(from, ebakusState)
			if minDv == nil || dv.Cmp(minDv) == -1 {
				minDv = dv
			}
		}
	}
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	return map[string]interface{}{
		...
		"delegateDiff":     head.DelegateDiff,
	}
}
...
func (s *PublicBlockChainAPI) rpcMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	// Add block producer address to the output.
	blockAuthor, err := s.b.GetBlockAuthor(b.Header())
	fields["producer"] = blockAuthor
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	if args.WorkNonce == nil {
		args.WorkNonce = new(hexutil.Uint64)
		*(*uint64)(args.WorkNonce) = 0
	}
	if args.Value == nil {
...
//
// https://github.com/ebakus/wiki/wiki/JSON-RPC#eth_sign
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice *float64, gasLimit *hexutil.Uint64) (common.Hash, error) {
...
func (s *PublicTransactionPoolAPI) GetAbiForAddress(ctx context.Context, addr common.Address) (string, error) {
	...
	if ebakusState == nil {
		return "", fmt.Errorf("Failed to find ebakusdb snapshot")
	}
	defer ebakusState.Release()

	return vm.GetAbiAtAddress(ebakusState, addr)
}

type PublicDBAPI struct {
	b                        Backend
	ebakusStateIteratorsMap  map[uint64]*list.Element
	ebakusStateIteratorsList *list.List
	ebakusStateIteratorsMux  sync.Mutex
}

// NewPublicTransactionPoolAPI creates a new RPC service with methods specific for the transaction pool.
func NewPublicDBAPI(b Backend) *PublicDBAPI {
	return &PublicDBAPI{b: b, ebakusStateIteratorsMap: make(map[uint64]*list.Element, 0), ebakusStateIteratorsList: list.New()}
}
...
func (api *PublicDBAPI) addEbakusStateIterator(tableName string, iter *ebakusdb.ResultIterator, contractAddress common.Address, blockNumber uint64) uint64 {
	...
	for {
		handle = rand.Uint64()
		if _, ok := api.ebakusStateIteratorsMap[handle]; !ok {
			break
		}
	}
	...
	if uint64(api.ebakusStateIteratorsList.Len()) > api.b.EbakusdbMaxActiveIterators() {
		oldestEntry := api.ebakusStateIteratorsList.Back()
		if oldestEntry != nil {
			oldestTableIter := oldestEntry.Value.(*ebakusStateIterator)
			api.releaseEbakusStateIterator(oldestTableIter.Handle)
		}
	}

	return tableIter.Handle
}
...
func (api *PublicDBAPI) getEbakusStateIterator(handle uint64) (*ebakusStateIterator, error) {
	...
	stateIterElem, ok := api.ebakusStateIteratorsMap[handle]
	if !ok {
		return nil, fmt.Errorf("Failed to find ebakusdb iterator")
	}

	stateIter, ok := stateIterElem.Value.(*ebakusStateIterator)
	if !ok {
		return nil, fmt.Errorf("Failed to find ebakusdb iterator")
	}
...
func (api *PublicDBAPI) Get(ctx context.Context, contractAddress common.Address, tableName string, whereClause string, orderClause string, blockNr rpc.BlockNumber) (interface{}, error) {
	...
	return vm.EbakusDBGet(ebakusState, contractAddress, tableName, whereClause, orderClause)
}
...
func (api *PublicDBAPI) Select(ctx context.Context, contractAddress common.Address, tableName string, whereClause string, orderClause string, blockNr rpc.BlockNumber) (hexutil.Uint64, error) {
	...
	iter, err := vm.EbakusDBSelect(ebakusState, contractAddress, tableName, whereClause, orderClause)
	if err != nil {
		return 0, err
	}

	handle := api.addEbakusStateIterator(tableName, iter, contractAddress, header.Number.Uint64())

	return hexutil.Uint64(handle), nil
}
...
func (api *PublicDBAPI) Next(ctx context.Context, iter hexutil.Uint64) (interface{}, error) {
	...
	if ret == nil {
		api.releaseEbakusStateIterator(uint64(iter))
	}

	return ret, nil
}

// ReleaseIterator releases table iterator
func (api *PublicDBAPI) ReleaseIterator(ctx context.Context, iter hexutil.Uint64) (interface{}, error) {
	api.releaseEbakusStateIterator(uint64(iter))
	return nil, nil
}
...
// PrivateDebugAPI is the collection of Ebakus APIs exposed over the private
// debugging endpoint.
...
// Version returns the current ebakus protocol version.
