...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ebakus is "ebakus". If no
...
func DefaultIPCEndpoint(clientIdentifier string) string {
	...
	path := DefaultDataDir()
	config := &Config{DataDir: path, IPCPath: clientIdentifier + ".ipc"}
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Ebakus", keep that.
	if name == "ebakus" || name == "ebakus-testnet" {
		name = "Ebakus"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "ebakus" instances.
var isOldEbakusResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by ebakus 1.4 are used if they exist.
	if warn, isOld := isOldEbakusResource[path]; isOld {
		oldpath := ""
		if c.name() == "ebakus" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldEbakusResourceWarning, "Using deprecated resource file %s, please move this file to the 'ebakus' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "ebakus-keystore")
		ephemeral = keydir
	}
