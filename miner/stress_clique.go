...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&ebakus); err != nil {
			panic(err)
		}
		if err := ebakus.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var ebakus *eth.Ebakus
		if err := nodes[index%len(nodes)].Service(&ebakus); err != nil {
			panic(err)
		}
		...
		if err := ebakus.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := ebakus.TxPool().Stats(); pend > 2048 {
...
func makeSealer(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Ebakus node
	...
	// Start the node and configure a full Ebakus node on it
