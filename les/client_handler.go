...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightEbakus) *clientHandler {
...
func (h *clientHandler) handle(p *peer) error {
	...
	p.Log().Debug("Light Ebakus peer connected", "name", p.Name())
	...
	if err := p.Handshake(big.NewInt(int64(number)), hash, number, h.backend.blockchain.Genesis().Hash(), nil); err != nil {
		p.Log().Debug("Light Ebakus handshake failed", "err", err)
		return err
	}
	// Register the peer locally
	if err := h.backend.peers.Register(p); err != nil {
		p.Log().Error("Light Ebakus peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("Light Ebakus message handling failed", "err", err)
			p.fcServer.DumpLogs()
			return err
		}
	}
...
func (h *clientHandler) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Ebakus message arrived", "code", msg.Code, "bytes", msg.Size)
