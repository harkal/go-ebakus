...
func (h *serverHandler) handle(p *peer) error {
	...
	if err := p.Handshake(big.NewInt(int64(number)), hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
	...
	if !h.server.clientPool.connect(p, 0) {
		p.Log().Debug("Light Ebakus peer registration failed", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light ebakus response", "err", err)
			return err
		default:
		}
