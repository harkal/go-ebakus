...
func (f *lightFetcher) nextRequest() (*distReq, uint64, bool) {
	...
	bestHash, bestAmount, bestNumber, bestSyncing = f.findBestRequest()
...
func (f *lightFetcher) findBestRequest() (bestHash common.Hash, bestAmount uint64, bestNumber *big.Int, bestSyncing bool) {
	...
	for p, fp := range f.peers {
		for hash, n := range fp.nodeByHash {
			...
			if (bestNumber == nil || n.number > bestNumber.Uint64() || amount < bestAmount) && (f.isTrustedHash(hash) || f.maxConfirmedNumber.Uint64() == 0) {
				bestHash = hash
				bestNumber = big.NewInt(int64(n.number))
				bestAmount = amount
...
func (f *lightFetcher) checkAnnouncedHeaders(fp *fetcherPeerInfo, headers []*types.Header, tds []*big.Int) bool {
	...
	for i := len(headers) - 1; ; i-- {
		if i < 0 {
			...
			td = new(big.Int).SetUint64(header.Number.Uint64() - 1)
			header = f.chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
			if header == nil {
				log.Error("Missing parent of validated header", "hash", header.ParentHash, "number", td)
				return false
			}
		} else {
			header = headers[i]
		...
		if n != nil {
			...
			if n.hash != hash || n.number != number || n.number != td.Uint64() {
...
func (f *lightFetcher) checkSyncedHeaders(p *peer) {
	...
	for node != nil {
		if td = big.NewInt(int64(node.number)); td != nil {
			break
		}
		node = node.parent
	}
...
func (f *lightFetcher) updateMaxConfirmedTd(td *big.Int) {
	if f.maxConfirmedNumber == nil || td.Cmp(f.maxConfirmedNumber) > 0 {
		f.maxConfirmedNumber = td
