...
import (
	...
	"github.com/ebakus/go-ebakus/eth/fetcher"
...
// NewProtocolManager returns a new Ebakus sub protocol manager. The Ebakus sub protocol manages peers capable
// with the Ebakus network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Ebakus peer", "peer", id)

	// Unregister the peer from the downloader and Ebakus peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Ebakus protocol")
	...
	log.Info("Ebakus protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Ebakus peer connected", "name", p.Name())

	// Execute the Ebakus handshake
	var (
	...
	if err := p.Handshake(pm.networkID, number, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Ebakus handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Ebakus peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Ebakus message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NewBlockMsg:
		...
		if err := block.Header().SanityCheck(); err != nil {
			return err
		}
		...
		if _, td := p.Head(); block.Number().Cmp(td) > 0 {
			p.SetHead(block.Hash(), block.Number())

			currentBlock := pm.blockchain.CurrentBlock()
			if block.NumberU64() > currentBlock.NumberU64()+10 {
				go pm.synchronise(p)
			}
		}
...
// NodeInfo represents a short summary of the Ebakus sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Ebakus network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
	HeadNumber *big.Int            `json:"headNumber"` // Head number of the host's blockchain
...
func (pm *ProtocolManager) NodeInfo() *NodeInfo {
	...
	return &NodeInfo{
		...
		HeadNumber: currentBlock.Number(),
