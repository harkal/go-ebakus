...
type Config struct {
	Blocks     int
	Percentile int
	Default    *float64 `toml:",omitempty"`
}
...
func (gpo *Oracle) SuggestPrice(ctx context.Context) (*float64, error) {
	gpo.cacheLock.RLock()
...
func (t transactionsByGasPrice) Less(i, j int) bool { return t[i].GasPrice() < t[j].GasPrice() }
...
func (gpo *Oracle) getBlockPrices(ctx context.Context, signer types.Signer, blockNum uint64, ch chan getBlockPricesResult) {
	...
	for _, tx := range txs {
		/*sender*/ _, err := types.Sender(signer, tx)
		if err == nil /*&& sender != block.Coinbase()*/ {
			gasPrice := tx.GasPrice()
			ch <- getBlockPricesResult{&gasPrice, nil}
			return
		}
	}
