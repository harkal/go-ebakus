...
import (
	...
	"github.com/ebakus/go-ebakus/eth/tracers"
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			/*
				coinbase, errr := api.eth.blockchain.Engine().Author(block.Header())
				if errr != nil {
					failed = errr
					break
				}
					...
					_, _, _, err := api.eth.blockchain.Processor().Process(block, statedb, coinbase, vm.Config{})
					if err != nil {
						failed = err
						break
					}
	...
	*/
	...
	/*
				for i, tx := range block.Transactions() {
					...
					vmenv := vm.NewEVM(vmctx, statedb, api.config)
		...
		>>>>>>> 07d2d83c315abae91659a28f75f8c42f6b9eb457
