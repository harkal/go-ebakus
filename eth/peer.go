...
var (
	propBlockOutTimer = metrics.GetOrRegisterTimer("eth/prop/blocks/out", nil)
)
...
// PeerInfo represents a short summary of the Ebakus sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Ebakus protocol version negotiated
	HeadNumber *big.Int `json:"headNumber"` // Head number of the peer's blockchain
...
func (p *peer) Head() (hash common.Hash, td *big.Int) {
	...
	copy(hash[:], p.head[:])
	return hash, new(big.Int).Set(p.HeadNumber)
}
...
func (p *peer) SendNewBlock(block *types.Block) error {
	defer propBlockOutTimer.UpdateSince(time.Now())
...
func (p *peer) AsyncSendNewBlock(block *types.Block) {
	select {
	case p.queuedProps <- &propEvent{block: block, td: big.NewInt(0)}:
		// Mark all the block hash as known, but ensure we don't overflow our limits
		p.knownBlocks.Add(block.Hash())
		for p.knownBlocks.Cardinality() >= maxKnownBlocks {
			p.knownBlocks.Pop()
...
func (p *peer) Handshake(network uint64, headNumber *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) error {
	...
	switch {
	case p.version == eth63:
		p.HeadNumber, p.head = status63.HeadNumber, status63.CurrentBlock
	case p.version == eth64:
		p.HeadNumber, p.head = status.HeadNumber, status.Head
	default:
		panic(fmt.Sprintf("unsupported eth protocol version: %d", p.version))
	}
	return nil
