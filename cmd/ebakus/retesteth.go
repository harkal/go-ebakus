...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches ebakus in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches ebakus in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func (e *NoRewardEngine) VerifyBlock(chain consensus.ChainReader, block *types.Block) error {
	return e.inner.VerifySeal(chain, block.Header())
}
...
func (e *NoRewardEngine) Prepare(chain consensus.ChainReader, stop <-chan struct{}) (*types.Block, *types.Header, error) {
...
func (e *NoRewardEngine) accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header) {
	...
	reward := big.NewInt(0)
	author, _ := e.Author(header)
	state.AddBalance(author, reward)
}

func (e *NoRewardEngine) Finalize(chain consensus.ChainReader, header *types.Header, statedb *state.StateDB, ebakusState *ebakusdb.Snapshot, coinbase common.Address, txs []*types.Transaction) {
	if e.rewardsOn {
		e.inner.Finalize(chain, header, statedb, ebakusState, coinbase, txs)
	} else {
...
func (e *NoRewardEngine) FinalizeAndAssemble(chain consensus.ChainReader, header *types.Header, statedb *state.StateDB, ebakusState *ebakusdb.Snapshot, coinbase common.Address, txs []*types.Transaction,
	receipts []*types.Receipt) (*types.Block, error) {
	if e.rewardsOn {
		return e.inner.FinalizeAndAssemble(chain, header, statedb, ebakusState, coinbase, txs, receipts)
	} else {
		e.accumulateRewards(chain.Config(), statedb, header)
		header.Root = statedb.IntermediateRoot(chain.Config().IsEIP158(header.Number))
...
func (api *RetestethAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(ethDb, ebakusDb, genesis)
	if err != nil {
		return false, err
	}
	...
	blockchain, err := core.NewBlockChain(ethDb, ebakusDb, nil, chainConfig, engine, vm.Config{}, nil)
	if err != nil {
		return false, err
	}
	...
	api.ebakusState = ebakusSnapshot
	api.blockNumber = 0
...
func (api *RetestethAPI) mineBlock() error {
	...
	gasLimit := core.CalcGasLimit(parent.Header(), 9223372036854775807, 9223372036854775807)
	header := &types.Header{
	...
	if api.engine != nil {
		...
		defer close(stopCh)
		api.engine.Prepare(api.blockchain, stopCh)
	}
	...
	for address := range api.txSenders {
		...
		for nonce := statedb.GetNonce(address); ; nonce++ {
			if tx, ok := m[nonce]; ok {
				...
				ebakusSnapshot := api.ebakusState.Snapshot()
				defer ebakusSnapshot.Release()
				...
				api.ebakusState.ResetTo(ebakusSnapshot)
				txs = append(txs, tx)
	...
	author, _ := api.engine.Author(header)
	block, err := api.engine.FinalizeAndAssemble(api.blockchain, header, statedb, api.ebakusState, author, txs, receipts)
	if err != nil {
		return err
	}
...
func (api *RetestethAPI) AccountRange(ctx context.Context,
	...
	if parentHeader == nil || int(txIndex) >= len(block.Transactions()) {
		...
		for idx, tx := range block.Transactions() {
			...
			vmenv := vm.NewEVM(context, statedb, api.ebakusState, api.blockchain.Config(), vm.Config{})
...
func (api *RetestethAPI) ClientVersion(ctx context.Context) (string, error) {
	return "Ebakus-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
