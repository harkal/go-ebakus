...
Use "ebakus dump 0" to dump the genesis block.`,
	}
...
func initGenesis(ctx *cli.Context) error {
	...
	ebakusDb, err := stack.OpenEbakusDatabase("state.db")
	if err != nil {
		utils.Fatalf("Failed to open ebakus state database: %v", err)
	}
	_, hash, err := core.SetupGenesisBlock(chaindb, ebakusDb, genesis)
	if err != nil {
