...
// Package utils contains internal helper functions for ebakus/go-ebakus commands.
package utils
...
import (
	...
	"github.com/ebakus/go-ebakus/metrics/influxdb"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 7=Testnet, 10=Mainnet)",
	...
	StorageFlag = cli.BoolFlag{
		Name:  "storage",
		Usage: "Ebakus private network used as backend DB layer",
	}
	...
	TxPoolPriceLimitFlag = cli.Float64Flag{
	...
	MinerGasPriceFlag = cli.Float64Flag{
		Name:  "miner.gasprice",
		Usage: "Minimum gas price for mining a transaction",
		Value: eth.DefaultConfig.Miner.GasPrice,
	}
	MinerLegacyGasPriceFlag = cli.Float64Flag{
		Name:  "gasprice",
...
// for Ebakus and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `ebakus account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func setTxPool(ctx *cli.Context, cfg *core.TxPoolConfig) {
	...
	if ctx.GlobalIsSet(TxPoolPriceLimitFlag.Name) {
		cfg.PriceLimit = ctx.GlobalFloat64(TxPoolPriceLimitFlag.Name)
	}
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	if ctx.GlobalIsSet(MinerLegacyGasPriceFlag.Name) {
		cfg.GasPrice = ctx.GlobalFloat64(MinerLegacyGasPriceFlag.Name)
	}
	if ctx.GlobalIsSet(MinerGasPriceFlag.Name) {
		cfg.GasPrice = ctx.GlobalFloat64(MinerGasPriceFlag.Name)
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, StorageFlag, TestnetFlag)
	CheckExclusive(ctx, StorageFlag, DeveloperFlag)
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		cfg.DPOS = *cfg.Genesis.Config.DPOS
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
	...
	case ctx.GlobalBool(StorageFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1340
		}
		// Create new developer account or reuse existing one
		...
		cfg.Genesis = core.DefaultStorageGenesisBlock(developer.Address)
		if !ctx.GlobalIsSet(MinerGasPriceFlag.Name) && !ctx.GlobalIsSet(MinerLegacyGasPriceFlag.Name) {
			cfg.Miner.GasPrice = types.MinimumTargetDifficulty
		}
	}
...
// RegisterEthService adds an Ebakus client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Ebakus service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Ebakus Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Ebakus
		ctx.Service(&ethServ)

		var lesServ *les.LightEbakus
		ctx.Service(&lesServ)

		// Let ethstats use whichever is not nil
		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Ebakus Stats service: %v", err)
	}
}
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return nil, errors.New("no Ebakus service")
	}); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "ebakus.", tagsMap)
		}
	}
}
...
func MakeEbakusDatabase(ctx *cli.Context, stack *node.Node) *ebakusdb.DB {
	db, err := stack.OpenEbakusDatabase("state.db")
	if err != nil {
		return nil
	}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(StorageFlag.Name):
		genesis = core.DefaultStorageGenesisBlock(common.Address{0})
	case ctx.GlobalBool(DeveloperFlag.Name):
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	ebakusDb := MakeEbakusDatabase(ctx, stack)

	genesis := MakeGenesis(ctx)
	config, _, err := core.SetupGenesisBlock(chainDb, ebakusDb, genesis)
	if err != nil {
		Fatalf("%v", err)
	}

	engine := dpos.New(config.DPOS, chainDb, ebakusDb, genesis)
	...
	chain, err = core.NewBlockChain(chainDb, ebakusDb, cache, config, engine, vmcfg, nil)
	if err != nil {
...
// new format.
//
// e.g. ebakus account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// ebakus --keystore /tmp/mykeystore --lightkdf account new
//
