...
func (w *wizard) makeGenesis() {
	...
	case choice == "3":
		...
		genesis.Config.DPOS.Period = uint64(w.readDefaultInt(5))
...
// importGenesis imports a Ebakus genesis spec into puppeth.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		// Export the native genesis spec used by puppeth and Ebakus
		ebakusJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((ebakusJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", ebakusJson)
