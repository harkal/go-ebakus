...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useEbakusV = true // Default to use V = 27 or 28, the legacy Ebakus format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useEbakusV, err
	}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Ebakus ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Ebakus Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useEbakusV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
