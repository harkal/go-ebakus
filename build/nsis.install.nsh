Name "ebakus ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install ebakus binary
Section "Ebakus" EBAKUS_IDX
  SetOutPath $INSTDIR
  file {{.Ebakus}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\ebakus.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\ebakus.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Ebakus incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Ebakus outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "Ebakus UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "Ebakus incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\ebakus.exe" "" "" "Ebakus" 30303 "" "" ""
  SimpleFC::AdvAddRule "Ebakus outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\ebakus.exe" "" "" "Ebakus" "" 30303 "" ""
  SimpleFC::AdvAddRule "Ebakus UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\ebakus.exe" "" "" "Ebakus" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ebakus/EIPs/issues/147)
  ${EnvVarUpdate} $0 "EBAKUS_SOCKET" "R" "HKLM" "\\.\pipe\ebakus.ipc"
  ${EnvVarUpdate} $0 "EBAKUS_SOCKET" "A" "HKLM" "\\.\pipe\ebakus.ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${EBAKUS_IDX}
    SectionGetSize ${EBAKUS_IDX} $0
