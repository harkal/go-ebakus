...
// Copyright 2018 The ebakus/go-ebakus Authors
...
export const percentPlotter = <T>(
	text: string,
	mapper: T => T = multiplier(1),
) => (payload: T) => {
...
export const bytePerSecPlotter = <T>(
	text: string,
	mapper: T => T = multiplier(1),
) => (payload: T) => {
		const p = mapper(payload);
...
export type Props = {
  ...
  tooltip: <T>(
    text: string,
    mapper?: (T) => T
  ) => (payload: mixed) => null | React$Element<any>,
};
