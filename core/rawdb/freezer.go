...
//   reserving it for ebakus/go-ebakus. This would also reduce the memory requirements
//   of Ebakus, and thus also GC overhead.
type freezer struct {
...
func (f *freezer) AppendAncient(number uint64, hash, header, body, receipts, ebakusSnapshot []byte) (err error) {
	...
	if err := f.tables[freezerSnapshotTable].Append(f.frozen, ebakusSnapshot); err != nil {
		log.Error("Failed to append ancient ebakusdb snapshot", "number", f.frozen, "hash", hash, "err", err)
		return err
	}
...
func (f *freezer) freeze(db ethdb.KeyValueStore) {
	...
	for {
		...
		for f.frozen < limit {
			...
			ebakusSnapshot := ReadSnapshotRLP(nfdb, hash, f.frozen)
			if len(ebakusSnapshot) == 0 {
				log.Error("Block ebakusdb snapshot missing, can't freeze", "number", f.frozen, "hash", hash)
				break
			}
			log.Trace("Deep froze ancient block", "number", f.frozen, "hash", hash)
			// Inject all the components into the relevant data tables
			if err := f.AppendAncient(f.frozen, hash[:], header, body, receipts, ebakusSnapshot); err != nil {
				break
			}
