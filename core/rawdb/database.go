...
func (db *nofreezedb) AppendAncient(number uint64, hash, header, body, receipts, ebakusSnapshot []byte) error {
...
func InspectDatabase(db ethdb.Database) error {
	...
	var (
		...
		snapshotSize    common.StorageSize
	...
	for it.Next() {
		...
		total += size
		switch {
		case bytes.HasPrefix(key, headerPrefix) && bytes.HasSuffix(key, snapshotSuffix):
			snapshotSize += size
	...
	for i, category := range []string{freezerHeaderTable, freezerBodiesTable, freezerReceiptTable, freezerHashTable, freezerSnapshotTable} {
	...
	stats := [][]string{
		...
		{"Key-Value store", "Snapshot", snapshotSize.String()},
