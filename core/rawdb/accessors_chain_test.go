...
// Tests ebakus state snapshot storage and retrieval operations.
func TestSnapshotStorage(t *testing.T) {
	db := ethdb.NewMemDatabase()

	// Create a test TD to move around the database and make sure it's really new
	hash, number, snapshot := common.Hash{}, uint64(1), uint64(314)
	if entry := ReadSnapshot(db, hash, number); entry != nil {
		t.Fatalf("Non existent Snapshot returned: %v", entry)
	}
	// Write and verify the Snapshot in the database
	WriteSnapshot(db, hash, snapshot)
	if entry := ReadSnapshot(db, hash, number); entry == nil {
		t.Fatalf("Stored Snapshot not found")
	}
	// Delete the Snapshot and verify the execution
	DeleteSnapshot(db, hash)
	if entry := ReadSnapshot(db, hash, number); entry != nil {
		t.Fatalf("Deleted Snapshot returned: %v", entry)
	}
}
...
func TestHeadStorage(t *testing.T) {
	...
	blockHead := types.NewBlockWithHeader(&types.Header{})
	blockFull := types.NewBlockWithHeader(&types.Header{})
	blockFast := types.NewBlockWithHeader(&types.Header{})
