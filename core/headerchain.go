...
func (hc *HeaderChain) WriteHeader(header *types.Header) (status WriteStatus, err error) {
	...
	ptd := hc.GetHeader(header.ParentHash, number-1).Number
	if ptd == nil {
		return NonStatTy, consensus.ErrUnknownAncestor
	}

	localTd := hc.CurrentHeader().Number
	externTd := new(big.Int).Add(header.Number, ptd)
...
// CurrentBlock in not applicable in this object
...
func (hc *HeaderChain) SetHead(head uint64, updateFn UpdateHeadBlocksCallback, delFn DeleteBlockContentCallback) {
	...
	for hdr := hc.CurrentHeader(); hdr != nil && hdr.Number.Uint64() > head; hdr = hc.CurrentHeader() {
		...
		rawdb.DeleteSnapshot(hc.chainDb, hash)
...
// StateAt is not valid in this situation
func (hc *HeaderChain) StateAt(hash common.Hash) (*state.StateDB, error) {
	return nil, fmt.Errorf("Unsupported on light chain")
}

// EbakusStateAt is not valid in this situation
func (hc *HeaderChain) EbakusStateAt(hash common.Hash, number uint64) (*ebakusdb.Snapshot, error) {
	return nil, fmt.Errorf("Unsupported on light chain")
}
