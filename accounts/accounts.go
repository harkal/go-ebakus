...
// Package accounts implements high level Ebakus account management.
package accounts
...
// Account represents an Ebakus account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Ebakus account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain ebakus.ChainStateReader)
...
//   keccak256("\x19Ebakus Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Ebakus Signed Message:\n%d%s", len(data), string(data))
