...
import (
	...
	"github.com/ebakus/go-ebakus/accounts/usbwallet/trezor"
...
// or whether the Ebakus app was not started on it.
...
// Ebakus address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EbakusAddress)
	if _, err := w.trezorExchange(&trezor.EbakusGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.EbakusTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EbakusTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
