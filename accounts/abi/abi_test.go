...
const tableUserDefinition = `[{"inputs":[{"name":"Id","type":"uint64","indexed":false},{"name":"Name","type":"string","indexed":false}],"name":"User","type":"table"}]`

func TestGetTableInstance(t *testing.T) {

	abi, err := JSON(strings.NewReader(tableUserDefinition))
	if err != nil {
		t.Fatal(err)
	}

	user, err := abi.GetTableInstance("User")
	if err != nil {
		t.Fatal(err)
	}
	...
	fv := v.FieldByName("Id")
	if !fv.IsValid() || fv.Type().String() != "uint64" {
		t.Fatal("Id: field not found or its type is incorrect")
	}

	fv = v.FieldByName("Name")
	if !fv.IsValid() || fv.Type().String() != "string" {
		t.Fatal("Name: field not found or its type is incorrect")
	}

	fv = v.FieldByName("NotExist")
	if fv.IsValid() {
		t.Fatal("Found a field that doesn't exist in struct")
	}
}

func TestTableUnpackSingle(t *testing.T) {

	expectedOutput := uint64(2)
	...
	input := common.LeftPadBytes([]byte{2}, 32)
	id, err := abi.UnpackSingle(user, "User", "Id", input)
	if err != nil {
		t.Fatal(err)
	}

	if id != expectedOutput {
		t.Fatalf("id is wrong %d, expected %d", id, expectedOutput)
	}

	v := reflect.Indirect(reflect.ValueOf(user))

	fv := v.FieldByName("Id")
	if !fv.IsValid() || fv.Type() != reflect.TypeOf(expectedOutput) || fv.Uint() != expectedOutput {
		t.Fatal("Id: field not found or type is incorrect")
	}

	if fv.Uint() != expectedOutput {
		t.Fatal("Id: value is not correct")
	}
}

func TestTablePack(t *testing.T) {

	expectedOutput := ""
	...
	sr := reflect.ValueOf(user)
	sr.Elem().FieldByName("Id").SetUint(uint64(1))
	sr.Elem().FieldByName("Name").SetString("test")

	out, err := abi.Pack("User", user)
	if err != nil {
		log.Fatalln(err)
	}

	if common.Bytes2Hex(out) != expectedOutput {
		t.Fatalf("Packed data are not correct. Output: %v", common.Bytes2Hex(out))
	}
}

func TestTableUnpack(t *testing.T) {

	inputData := ""
	...
	err = abi.Unpack(user, "User", common.Hex2Bytes(inputData))
	if err != nil {
		t.Fatal(err)
	}
	...
	fv := v.FieldByName("Id")
	if fv.Uint() != uint64(1) {
		t.Fatalf("Id value was wrong: %d", fv.Uint())
	}

	fv = v.FieldByName("Name")
	if fv.String() != "test" {
		t.Fatalf("Name value was wrong: %v", fv.String())
	}
}
